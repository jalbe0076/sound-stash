describe('Search', () => {
  ...
  it('As a user I should see an search bar, type Outkast, click the search button, and see 49 results each with an image, title, and artist', () => {
    cy.wait('@trending').then(() => {
      cy.get('[value="user2"]').click()
        ...
        .get('.album--grid').find('.results--card').should('have.length', '49')
  ...
  it('As a user I should see an search bar, type Outkast, see some results, and click the next button to see more results.  I should also be able to click the last button and see the previous results', () => {
    cy.wait('@trending').then(() => {
      cy.get('[value="user2"]').click()
        ...
        .get('.album--grid').find('.results--card').should('have.length', '23')
  ...
  it('If there are no users I should be able to search for vinyl albums and see an error message if there are no search results', () => {
    cy.wait('@trending').then(() => {
  ...
  it('If no user is logged in, type Outkast, click the search button, and see 49 results each with an image, title, and artist', () => {
    cy.wait('@trending').then(() => {
      cy.get('[value="no-user"]').click()
        .url().should('include', '/')
        .get('.search--form').should('be.visible')
